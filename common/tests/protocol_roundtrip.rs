...
fn chat_request_roundtrip() {
    let req = ChatRequest {
        ...
        messages: vec![
            ChatMessage { images: vec![], tool_calls: vec![], tool_call_id: None, role: "system".into(), content: "You are helpful.".into() },
            ChatMessage { images: vec![], tool_calls: vec![], tool_call_id: None, role: "user".into(), content: "Hello".into() },
        ],
...
fn chat_request_no_temperature_roundtrip() {
    let req = ChatRequest {
        model: "llama3:latest".into(),
        messages: vec![ChatMessage { images: vec![], tool_calls: vec![], tool_call_id: None, role: "user".into(), content: "Hi".into() }],
        temperature: None,
        stream: false,
        think: None,
        tools: vec![],
...
fn chat_response_roundtrip() {
    let resp = ChatResponse {
        message: ChatMessage { images: vec![], tool_calls: vec![], tool_call_id: None, role: "assistant".into(), content: "Hello!".into() },
    };
...
fn response_chat_with_metering_roundtrip() {
    let resp = Response::Chat {
        result: ChatResponse {
            message: ChatMessage { images: vec![], tool_calls: vec![], tool_call_id: None, role: "assistant".into(), content: "Hi!".into() },
        },
        metering: Metering {
            prompt_tokens: 20,
            completion_tokens: 5,
